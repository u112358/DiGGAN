...
class GaitGAN:
    def __init__(self, tf_config):
        ...
        self.control_angle = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, self.num_views),
                                            name='control_angle')
        self.control_angle_strong = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, self.num_views),
                                                   name='control_angle_strong')
        self.control_angle_label = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, 1),
                                                  name='control_angle_label')
        ...
        self.logits_true_for_d_angle, _ = layers.conditional_discriminator(self.input, self.control_angle_strong,
        ...
        self.D_angle_loss_true = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.control_angle_label,
                                                                                        logits=self.logits_true_for_d_angle))
    ...
    def train(self, k):
        with tf.Session(config=self.tf_config) as sess:
            ...
            for step in tqdm(range(self.max_step)):
                #
                # x, angle = dr.get_next_batch_with_random_angle(self.batch_size)
                # x = np.expand_dims(np.array(x), -1)
                # angle = np.squeeze(angle, 1)
                # angle_list = np.random.randint(0, 14, self.batch_size)
                angle_list = np.ones((self.batch_size,),dtype=np.int32) * step / 2900.0
                ...
                angle_strong = np.copy(angle)
                angle_strong_label = np.zeros(self.batch_size)
                random_angle = np.random.randint(0, self.num_views, int(self.batch_size / 2))
                for i, _a in enumerate(random_angle):
                    temp = np.zeros(self.num_views)
                    temp[_a] = 1
                    angle_strong[i] = temp
                flag_list = np.equal(angle, angle_strong).tolist()
                for i, flag in enumerate(flag_list):
                    if all(flag):
                        angle_strong_label[i] = 1
                    else:
                        angle_strong_label[i] = 0
                angle_strong_label = np.expand_dims(angle_strong_label, -1)
                for j in range(k):
                    # training on d_id
                    _, _, loss_D, loss_D_angle = sess.run(
                        [self.D_id_train, self.D_angle_train, self.D_id_loss, self.D_angle_loss],
                        feed_dict={self.input: x, self.paired_input: paired_x, self.control_angle: angle,
                                   self.control_angle_label: angle_strong_label,
                                   self.control_angle_strong: angle_strong})

                    # training on d_angle
                    # _, loss_D_angle = sess.run([self.D_angle_train, self.D_angle_loss],
                ...
                _, loss_EG, sum = sess.run([self.EG_train, self.G_loss, self.summary_op_general],
                                           feed_dict={self.input: x, self.control_angle: angle,
                                                      ...
                                                      self.control_angle_label: angle_strong_label,
                                                      self.control_angle_strong: angle_strong})
                print("Step: %d\t Loss_G: %lf\t  Loss_D: %lf\t Loss_D_angle: %lf" % (
                    step, loss_EG, loss_D, loss_D_angle))
                ...
                if (step + 1) % 1000 == 0:
                    ...
                    sum = sess.run(self.summary_op_validate, feed_dict={self.result_input: self.result_txt,
                                                                        self.acc_input: self.form_acc_to_feed()})
                    self.summary_writer.add_summary(sum, step)
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            saver.restore(sess=sess, save_path='./exp/model/20190928-143143/-36999')
            dr = DataReader('./GEI/GEI')
            test_input_angle = [0, 30, 60, 90]
            for i in range(4):
                x = dr.get_test_input(i*2)
                x = np.expand_dims(x, 0)
                x = np.expand_dims(x, -1)
                for j in range(14):
                    _angle = np.zeros(14)
                    _angle[j] = 1
                    _angle = np.reshape(_angle, (1, -1))
                    img = sess.run(self.generated_img, feed_dict={self.input: x, self.control_angle: _angle})
                    _filepath = '%d_%d.jpg' % (i, j)
                    img = np.squeeze(img, 0)
                    img = np.squeeze(img, -1)
                    imsave(_filepath, img)
