...
class GaitGAN:
    ...
    def train(self, k):
        with tf.Session(config=self.tf_config) as sess:
            ...
            # self.saver.restore(sess, './exp/model/20190902-023506/-18999')
    ...
    def validation_for_verification(self, sess, data_reader):
        angles = [0, 30, 60, 90]
        acc = np.zeros((4, 4))
        with tqdm(total=16) as pbar:
            for i in range(4):
                for j in range(4):
                    acc[i, j] = self.verify_probe_n_at_gallery_m(data_reader, sess, angles[i], angles[j])
                    pbar.update(1)
        print_acc(acc)
        result_txt = self.plot_acc(acc, sess)
        return acc, result_txt
