...
class GaitGAN:
    def __init__(self, tf_config):
        ...
        # self.z = layers.encoder_old_school(self.input, self.embedding_dims)
        self.z = layers.encoder_deep_cnn(self.input, self.embedding_dims)
        ...
        self.learning_rate = tf.compat.v1.train.exponential_decay(self.init_learning_rate, self.global_step, 10000, 0.5, staircase=True)
    ...
    def train(self, k):
        with tf.Session(config=self.tf_config) as sess:
            ...
            # self.saver.restore(sess, './exp/model/20190903-160112/-26999')
            ...
            for step in tqdm(range(self.max_step)):
                ...
                if nof_triplet > 0:
                    ...
                    _, loss, sum = sess.run([self.train_op, self.loss, self.sum_op],
                                            feed_dict={self.input: x, self.len_triplet: nof_triplet,
                                                       self.distance_neg: dist_n, self.distance_pos: dist_p})
                    print('step:%d, loss:%lf, nof_triplets:%d' % (step, loss, nof_triplet))
